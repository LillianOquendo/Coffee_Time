...
#Do you want to view shops near you or view shops by rating
    ...
    if filterby == '1':
        ...
        shop_list = session.query(Shop).filter(Shop.shop_location_id == int(user_location))   #list of records where shop location matches the users location.
        print(shop_list)
    ...
    coffee_list_id = session.query(ShopMenu).filter(ShopMenu.shop_id == shop_id) #Going through the new shop menu table and finding coffee id a list. this new table feels unneccesary and just adds more searching for no reason. Have to know go through this list of coffee id to get the names of the coffee?
